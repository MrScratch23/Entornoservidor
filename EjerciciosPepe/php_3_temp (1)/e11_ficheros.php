...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['enviar'])) {
    ...
    $nombre_archivo = htmlspecialchars(trim($_POST['nombre']??'')); // Obtener el nombre del archivo
    $fichero = $_FILES['fichero']; // Obtener la información del archivo

    // Validar el nombre del archivo
    if ($nombre_archivo==='' || strlen($nombre_archivo) < 3 || strlen($nombre_archivo) > 30) {
    ...
    if (empty($errores)) {
        if (move_uploaded_file($fichero['tmp_name'], $ruta_fichero)) {
            ...
            //sort($palabras);
            //para ordenarlo sin tener en cuenta mayúsculas/minúsculas
            natcasesort($palabras);
            // Guardar las palabras en el archivo
            file_put_contents($ruta_fichero, implode("\n", $palabras));
        } else {
